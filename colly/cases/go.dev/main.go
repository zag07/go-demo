...
var (
	...
	metaInfo   = "../../storage/go.dev/meta.json"
	storageDir = "../../storage/go.dev/"
	blogs      = make([]blog, 0)
...
func init() {
	...
	_ = os.MkdirAll(storageDir+"css/", 0755)
	_ = os.MkdirAll(storageDir+"assets/", 0755)

	err = ujson.JsonLoad(metaInfo, &oldBlogs)
	if err != nil {
	...
	c.OnHTML("div[id='blogindex']", func(e *colly.HTMLElement) {
		e.ForEach("p[class='blogtitle']", func(_ int, e *colly.HTMLElement) {
			...
			i := searchBlog(paths[len(paths)-1], blogs)
			if i == -1 {
...
func main() {
	...
	c.OnHTML("link[rel='stylesheet']", func(e *colly.HTMLElement) {
		...
		fileName := storageDir + "css/" + paths[len(paths)-1]
	...
	blogCollector.OnHTML("html", func(e *colly.HTMLElement) {
		...
		fileName := storageDir + fmt.Sprintf("%03d_", prefixInt) + paths[len(paths)-1] + ".html"
		f, err := os.Create(fileName)
		if err != nil {
	...
	blogCollector.OnHTML("img[src]", func(e *colly.HTMLElement) {
		...
		_ = os.MkdirAll(storageDir+"assets/"+paths[len(paths)-2], 0755)
		fileName := storageDir + "assets/" + paths[len(paths)-2] + "/" + paths[len(paths)-1]
		if _, err := os.Stat(fileName); err == nil {
			return
		}
